...
### Le widget : à faire avant le prochain envoi TestFlight

Le widget d'écran d'accueil lit ce que l'app lui écrit dans un conteneur partagé. Il faut donc, une fois, dans Certificates, Identifiers & Profiles :

1. **App Group** : Identifiers → App Groups → `group.uk.riskybusinesses.tangent`.
2. **App ID `uk.riskybusinesses.tangent`** : cocher la capacité App Groups et lui rattacher le groupe.
3. **Nouvel App ID** `uk.riskybusinesses.tangent.widget`, même capacité, même groupe.
