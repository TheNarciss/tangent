      ...
      <CardContent className="space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <NumberField label="Versement mensuel (€)" value={monthly}
                       ...
                       min={0} step={50} />
          <NumberField label="Horizon (années)" value={years}
...
function FanChart({ data }: FanProps) {
  ...
  const envelopePath = (lo: number[], hi: number[]) => {
    ...
    for (let i = hi.length - 1; i >= 0; i--) p += `L${xAt(i).toFixed(2)},${yScale(hi[i]).toFixed(2)} `;
    return p + "Z";
  };
    ...
    <div ref={wrapRef} className="relative w-full">
      ...
      >
        ...
        <g stroke="hsl(var(--border))" strokeWidth="0.5" opacity="0.5">
          {yTicks.map((t) => (
            <line key={`yg-${t}`} x1={PAD.left} x2={PAD.left + innerW} y1={yScale(t)} y2={yScale(t)} />
          ))}
        </g>

        {/* P10-P90 envelope */}
        <path d={envelopePath(data.bands.p10, data.bands.p90)} fill={COLOR.band} fillOpacity={0.08} />
        {/* P25-P75 envelope */}
        <path d={envelopePath(data.bands.p25, data.bands.p75)} fill={COLOR.band} fillOpacity={0.14} />
        ...
        {goalPx !== null && (
          <g>
            <line
              ...
              strokeWidth="1"
            />
            <text x={PAD.left + innerW - 4} y={goalPx - 4} textAnchor="end" className="font-mono text-[10px]"
                  fill={COLOR.goal}>
        ...
        {/* Deterministic lines */}
        <path d={linePath(data.bands.bear, xAt, yScale)} fill="none" stroke={COLOR.bear} strokeWidth="1.4" strokeOpacity={0.85} />
        ...
        <g className="font-mono text-[10px] fill-current text-muted-foreground">
          {xTicks.map((i) => (
            <g key={`xt-${i}`}>
              <line x1={xAt(i)} x2={xAt(i)} y1={PAD.top + innerH} y2={PAD.top + innerH + 4} stroke="currentColor" />
              <text x={xAt(i)} y={PAD.top + innerH + 16} textAnchor="middle">
                {data.months[i] / 12 < 1 ? `${data.months[i]}m` : `${Math.round(data.months[i] / 12)}a`}
              </text>
            </g>
          ))}
          <text x={PAD.left + innerW / 2} y={H - 6} textAnchor="middle" className="font-sans text-xs">
            Horizon
          </text>
        </g>
        ...
        {hoverIdx !== null && (
          <g>
            <line
              ...
              opacity={0.5}
            />
            <circle cx={xAt(hoverIdx)} cy={yScale(data.bands.p50[hoverIdx])} r="4" fill={COLOR.base} />
          </g>
        )}
      </svg>

      {hoverIdx !== null && (
        <ChartTooltip x={(xAt(hoverIdx) / W) * wrapW} y={(yScale(data.bands.p50[hoverIdx]) / H) * (wrapW * H / W)} containerWidth={wrapW}>
...
function Band({ color }: { color: string }) {
  return (
    <span className="relative inline-block h-3 w-6 overflow-hidden">
      ...
      <span className="absolute inset-y-0 left-1 right-1 opacity-40" style={{ background: color }} />
    </span>
  );
}
