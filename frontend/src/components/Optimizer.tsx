...
const OBJECTIVE_DESCRIPTIONS: Record<OptimizerObjective, string> = {
  from_strategy: "Maximise Sharpe sous tes contraintes σ ≤ vol max ET μ ≥ rendement cible (depuis ton profil). Te dit si ton intention est atteignable.",
  target_volatility: "Maximise μ sous contrainte σ_p ≤ cible. C'est ici que livrets et ETFs se mixent vraiment.",
  max_sharpe: "Maximise (μ − r_f) / σ. Sans contrainte de risque, l'optimiseur sature les actifs à plus haut ratio.",
...
export function Optimizer({
  ...
  includeEnvelopes, onIncludeEnvelopesChange,
  totalCapital, onTotalCapitalChange,
  maxVolatility, onMaxVolatilityChange,
  hasProfile, query,
  profileTargetReturn, profileMaxVol,
}: Props) {
  return (
    <Card>
      <CardHeader>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 items-end">
          ...
          {objective === "target_volatility" && (
            <NumberInput label="σ max (% /an)" value={maxVolatility} onChange={onMaxVolatilityChange}
          ...
          {objective === "from_strategy" && (
            <div className="space-y-1 col-span-1 sm:col-span-2 lg:col-span-1">
              ...
              <div className="h-9 flex items-center px-3 rounded-md border bg-card/50 text-xs font-mono tabular">
                {hasProfile ? (
                  <span>μ ≥ <strong>{profileTargetReturn}%</strong> · σ ≤ <strong>{profileMaxVol}%</strong></span>
                ) : (
                  <span className="text-[hsl(var(--loss))]">Renseigne ton profil</span>
                )}
              </div>
              <p className="text-[10px] text-muted-foreground">Modifie via l'icône profil en haut</p>
            </div>
          )}

          <ToggleField label="Inclure mes livrets" checked={includeEnvelopes}
                       onChange={onIncludeEnvelopesChange} disabled={!hasProfile}
                       hint={hasProfile ? "Selon ton profil + plafonds restants" : "Renseigne ton profil d'abord"} />

          <NumberInput label="Capital total (€)" value={totalCapital} onChange={onTotalCapitalChange}
                       min={0} step={1000}
                       hint="Pool sur lequel répartir. Défaut = portefeuille actuel" allowEmpty />
        </div>
          ...
          <>
            ...
            {includeEnvelopes && objective === "max_sharpe" && (
              <p className="text-xs text-muted-foreground italic border-l-2 border-muted pl-3">
                Note : avec livrets et <em>max Sharpe</em> sans contrainte de risque, le solveur sature les livrets
                (σ ≈ 0, Sharpe ≈ ∞). Honnête mais peu nuancé. Passe en <strong>« Selon ma stratégie »</strong> ou
...
function NumberInput({ label, value, onChange, min, max, step, hint, allowEmpty }: {
  label: string; value: number | ""; onChange: (v: number | "") => void;
  min?: number; max?: number; step?: number; hint?: string; allowEmpty?: boolean;
}) {
  return (
    <div className="space-y-1">
    ...
    <div className="space-y-1">
      ...
      <button type="button" disabled={disabled}
              ...
              className={cn(
                ...
                disabled ? "opacity-40 cursor-not-allowed bg-muted/30" :
                checked ? "bg-[hsl(var(--gain))]/15 border-[hsl(var(--gain))] text-foreground"
                        : "hover:bg-muted/40",
              )}>
...
function CompareRow({ label, current, optimal, delta, sign, bold }: {
  label: string; current: string; optimal: string; delta: string; sign: number; bold?: boolean;
}) {
  const color = Math.abs(sign) < 1e-6 ? "text-muted-foreground"
              : sign > 0 ? "text-[hsl(var(--gain))]" : "text-[hsl(var(--loss))]";
  return (
      ...
      <ul className="space-y-2">
        {significantActions.map((a) => {
          ...
          const verb = isEnvelope
            ? "Place"
            : sign === "buy" ? "Achète" : sign === "sell" ? "Vends" : "Conserve";
          const color = isEnvelope
            ? "text-[hsl(160_64%_50%)]"
            : sign === "buy" ? "text-[hsl(var(--gain))]"
            : sign === "sell" ? "text-[hsl(var(--loss))]" : "text-muted-foreground";
          ...
          return (
            <li key={a.ticker}
                className={cn(
                  ...
                  isEnvelope ? "bg-[hsl(160_64%_50%)]/5 border-[hsl(160_64%_50%)]/30"
                             : "bg-card/50",
                )}>
              <div className="flex items-center gap-2 font-mono text-sm min-w-0">
                <span className={cn("inline-block px-1.5 py-0.5 rounded text-[10px] uppercase tracking-wider",
                                    isEnvelope ? "bg-[hsl(160_64%_50%)]/20 text-[hsl(160_64%_50%)]"
                                               : "bg-muted text-muted-foreground")}>
                  {isEnvelope ? "Livret" : "ETF"}
                </span>
                <span className="font-medium truncate">{label}</span>
                <span className="text-muted-foreground tabular text-xs">{fmt.pct(a.current_weight)}</span>
              ...
              <div className={cn("flex items-center gap-1.5 text-sm font-mono tabular", color)}>
                ...
                <span>{verb} {fmt.eur(isEnvelope ? optAmount : deltaAmount)}</span>
              </div>
            </li>
          );
        })}
...
function RiskBar({ label, rc, kinds, labels }: {
  label: string; rc: RiskContribution; kinds: ("etf" | "envelope")[]; labels: string[];
}) {
  let etfIdx = 0;
  return (
    <div className="space-y-1">
      ...
      <div className="flex h-6 w-full rounded overflow-hidden border bg-muted/20">
        {rc.fraction.map((f, i) => {
          ...
          const kind = kinds[i];
          const color = kind === "envelope" ? ENVELOPE_COLOR : ETF_COLORS[etfIdx++ % ETF_COLORS.length];
          return (
            <div key={labels[i] + i}
