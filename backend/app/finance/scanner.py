...
_MODE_BUILDERS: dict[str, Callable[[], EquityQuery]] = {
    ...
    "tech_growth": lambda: EquityQuery(
        "and",
        [
            EquityQuery("eq", ["exchange", "PAR"]),
...
def _compute_delta_sharpe(
    ...
    new_sigma_sq = (
        (1 - h) ** 2 * p_sigma**2 + h**2 * sigma_c**2 + 2 * h * (1 - h) * rho * p_sigma * sigma_c
    )
...
def scan(req: ScanRequest, portfolio_data=None) -> ScanResponse:
    ...
    pf_stats = analytics.portfolio_stats(
        returns_p, weights, risk_free=rf, mu_override=mu_override_p
    )
    p_mu, p_sigma, p_sharpe = (
        pf_stats["expected_return"],
        pf_stats["volatility"],
        pf_stats["sharpe"],
    )
    ...
    candidates = [
        c
        for c in raw
        if c.get("symbol") and _is_pea_eligible(c["symbol"]) and c["symbol"] not in in_portfolio
    ]
    if not candidates:
        return ScanResponse(
            candidates=[],
