...
from app.data import damodaran, ecb, eurostat, fred, http, ken_french, lbma, openfigi, shiller
...
def _damodaran_sheet() -> "pd.DataFrame":
    return pd.DataFrame(
        {
            "Year": ["1928", "2008", "2025", "Average", None],
            "S&P 500 (includes dividends)": [0.4381, -0.3649, 0.25, 0.11, None],
            " Baa Corporate Bond": [0.0322, -0.0507, 0.05, 0.06, None],
        }
    )


def test_damodaran_keeps_only_the_year_rows(monkeypatch):
    monkeypatch.setattr(damodaran, "_sheet", _damodaran_sheet)

    annual = damodaran.annual_returns("equity_us")

    assert len(annual) == 3  # "Average" and the empty row are dropped
    assert annual.index[0].year == 1928
    assert annual.loc["2008"].iloc[0] == pytest.approx(-0.3649)


def test_damodaran_matches_a_column_despite_its_stray_space(monkeypatch):
    monkeypatch.setattr(damodaran, "_sheet", _damodaran_sheet)

    # The workbook writes " Baa Corporate Bond" with a leading space.
    assert damodaran.window_return("corporate_baa", 2008, 2008) == pytest.approx(-0.0507)


def test_damodaran_says_so_when_the_workbook_is_reorganized(monkeypatch):
    monkeypatch.setattr(damodaran, "_sheet", lambda: pd.DataFrame({"Year": ["2008"], "X": [0.1]}))

    with pytest.raises(DataSourceError, match="réorganisé"):
        damodaran.annual_returns("equity_us")


def _shiller_sheet() -> "pd.DataFrame":
    rows = [[None] * 10 for _ in range(8)]
    rows[4][9], rows[5][9], rows[6][9], rows[7][9] = "Real", "Total", "Return", "Price"
    rows[4][4], rows[5][4], rows[6][4], rows[7][4] = "Consumer", "Price", "Index", "CPI"
    body = [
        [1871.01] + [None] * 8 + [100.0],
        [1871.1] + [None] * 8 + [110.0],  # 1871.1 is October, not January
        [1872.01] + [None] * 8 + [121.0],
    ]
    return pd.DataFrame(rows + body)


def test_shiller_reads_october_as_october(monkeypatch):
    monkeypatch.setattr(shiller, "_sheet", _shiller_sheet)

    levels = shiller.series("real_total_return")

    assert [d.strftime("%Y-%m") for d in levels.index] == ["1871-01", "1871-10", "1872-01"]


def test_shiller_computes_a_window_return(monkeypatch):
    monkeypatch.setattr(shiller, "_sheet", _shiller_sheet)

    assert shiller.window_return("real_total_return", "1871-01", "1872-01") == pytest.approx(0.21)


def test_shiller_refuses_a_column_whose_header_moved(monkeypatch):
    shifted = _shiller_sheet()
    shifted.iat[4, 9] = "Nominal"
    shifted.iat[6, 9] = "Something"
    monkeypatch.setattr(shiller, "_sheet", lambda: shifted)

    with pytest.raises(DataSourceError, match="classeur a changé"):
        shiller.series("real_total_return")
